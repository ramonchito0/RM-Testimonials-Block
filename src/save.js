...
export default function save({ attributes }) {
  ...
  return (
    <div
      {...useBlockProps.save({
        className: `${style == "default" ? "rm-bg-secondary" : "rm-bg-white"} rm-testimonials-block`,
      })}
    >
      <div className="rm-container rm-flex rm-flex-col rm-gap-14 rm-py-14 lg:rm-py-20 2xl:rm-px-0">
        ...
        <div className="dark:text-primary-foreground rm-grid rm-grid-cols-1 rm-gap-24 md:rm-grid-cols-2">
          {testimonials.map((testimonial, index) => (
            <div
              key={index}
              className={clsx(
                "",
                `${style == "default" ? "rm-bg-white" : "rm-bg-secondary"}`,
                `${style} testi-stag-up rm-space-y-8 rm-rounded-3xl  rm-px-8 rm-pb-10 rm-text-center`,
              )}
            >
              {testimonial.image && (
